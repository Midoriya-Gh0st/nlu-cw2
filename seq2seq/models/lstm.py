...
class LSTMDecoder(Seq2SeqDecoder):
    ...
    def forward(self, tgt_inputs, encoder_out, incremental_state=None):
        ...
        if cached_state is not None:
            ...
            tgt_hidden_states = [torch.zeros(tgt_inputs.size()[0], self.hidden_size) for i in range(len(self.layers))] #- tgt_hidden_states size: List(torch.Size([10, 128]))
            print('tgt_hidden_states size:',tgt_hidden_states[0].size())
            input()
            tgt_cell_states = [torch.zeros(tgt_inputs.size()[0], self.hidden_size) for i in range(len(self.layers))] #- tgt_cell_states size:List(1)
            print('tgt_cell_states size:',len(tgt_cell_states))
            input_feed = tgt_embeddings.data.new(batch_size, self.hidden_size).zero_()#- input_feed size:List(10)
